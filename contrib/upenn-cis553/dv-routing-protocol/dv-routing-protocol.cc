...
DVRoutingProtocol::GetTypeId(void)
{
  static TypeId tid = TypeId("DVRoutingProtocol")
                          ...
                          .AddAttribute("HelloTimeout", 
                                        "Frequency for HELLO_REQ in milliseconds", 
                                        TimeValue(MilliSeconds(2000)),
...
void DVRoutingProtocol::DumpNeighbors()
{
  ...
  */
  ...
  while (it != m_neighborTable.end())
  {
    ...
    // STATUS_LOG(it->first << "\t\t" << it->second.first << "\t\t" << it->second.second << std::endl);
...
void DVRoutingProtocol::ProcessHelloRsp(DVMessage dvMessage, Ipv4Address interfaceAddr)
{
  // Check destination address
  ...
  {    
    ...
    TRAFFIC_LOG("Received HELLO_RSP, From Node: " << ReverseLookup(dvMessage.GetOriginatorAddress()) 
...
void DVRoutingProtocol::SayHello()
{
...
void DVRoutingProtocol::SetIpv4(Ptr<Ipv4> ipv4)
{
  ...
  m_helloTimer.SetFunction(&DVRoutingProtocol::SayHello, this);
  m_ipv4 = ipv4;
