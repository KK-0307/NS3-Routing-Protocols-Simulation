...
public:
  ...
  void ProcessHelloRsp(LSMessage lsMessage, Ipv4Address interfaceAddr);
  void ProcessLSFlood(LSMessage lsMessage, Ipv4Address interfaceAddr);
  ...
  // Flooding Function
  void Flood(LSMessage lsMessage);
...
private:
  ...
  // Neighbor Table
  // Node Number --> (Neighbor Address, Interface Address)
  std::map<std::string, std::tuple<Ipv4Address, Ipv4Address>> m_neighborTable;

  // Neighbor Log
  // Node Number --> Timestamp
  std::map<std::string, Time> m_neighborLog;

  // Routing Table
  // Node Number --> (Next Hop, Next Hop Interface Address, Cost)
  std::map<std::string, std::tuple<Ipv4Address, int>> m_routingTable;

  // LSP Table
  // Node Number --> (Sequence Number, Neighbours)
  std::map<std::string, std::tuple<uint64_t, std::vector<Ipv4Address>>> m_validLSP;
};
#endif